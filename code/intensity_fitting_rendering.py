...
def save_img(intensity_multi, mask_list, model_name):
    l_num = len(intensity_multi)
    for i in range(l_num):
        mask = mask_list[i]
        scene = os.path.splitext(os.path.basename(img_files[i]))[0]
        plt.imsave(SAVE_PATH + f'intensity/{scene}_{model_name}.png', intensity_multi[i]*mask, vmax=VMAX_INTENSITY, cmap='gray')

def save_intensity_error(intensity_multi, renderer, mask_list):
    ...
    for i in range(l_num):
        ...
        errormap = np.abs(intensity_multi[i] - renderer.intensity_multi[i])
...
def save_intensity_plot(intensity_multi, mask_list, model_name, n, l_list, ylims, xlim=[0, 90]):
    l_num = len(intensity_multi)
    for i in range(l_num):
        mask = mask_list[i]
        scene = os.path.splitext(os.path.basename(img_files[i]))[0]
        ...
        plt.figure(dpi=100)
        plot_x = (180/np.pi)*np.arccos(np.clip(np.sum(n*l, axis=-1), 0.0, 1.0))
        plot_x = plot_x[mask]
        plot_y = intensity_multi[i, mask]
        plt.scatter(plot_x, plot_y, s=1, c=color_dict[model_name])
        plt.xlabel(r'$cos^{-1} (N \cdot L)$')
        plt.ylabel('intensity')
        plt.xlim(xlim)
        plt.ylim(ylim)
        plt.savefig(SAVE_PATH + f'intensity_plot/{scene}_nl_{model_name}.png', bbox_inches="tight")
        plt.close()
...
os.makedirs(SAVE_PATH+'intensity_plot/', exist_ok=True)
...
for i in range(l_num):
    ...
    ylims[i,1] = np.quantile(intensity_multi[i, mask], 0.9999) + 0.1

save_img(intensity_multi, mask_list_rendering, 'observation')
save_intensity_plot(intensity_multi, mask_list_rendering, 'observation', n_GT, l_list, ylims=ylims)

# load estimated parameters
model_name = 'FMBRDF'
LOAD_PATH = DATA_PATH+obj_name+f'/fitting-intensity/'
...
save_img(renderer_BRDF.intensity_multi, mask_list_rendering, model_name)
save_intensity_error(intensity_multi, renderer_BRDF, mask_list_error)
save_intensity_plot(renderer_BRDF.intensity_multi, mask_list_rendering, model_name, n_GT, l_list, ylims=ylims)